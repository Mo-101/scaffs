...
from research.data_store import DATA_DIR, load_bars
...
_UNIVERSE_TAG = {
    ...
    "sp500": "equity_us",
    "btc-usdt": "crypto",
    "binance_perps_34": "crypto",
}

# 1h Binance USD-M perp symbols taken from agent/universe_50.txt.
# The loader prunes to the subset with sufficient history for the period.
_BINANCE_PERPS_34_TIMEFRAME = "1h"
_BINANCE_PERPS_MIN_COVERAGE = 0.9
_BINANCE_PERPS_34_UNIVERSE_FILE = Path(__file__).resolve().parents[2] / "universe_50.txt"
...
def _load_universe_panel(
    ...
    Args:
        universe: ``csi300`` | ``sp500`` | ``btc-usdt`` | ``binance_perps_34``.
    ...
    elif universe == "binance_perps_34":
        panel = _load_binance_perps_34_panel(start, end)
...
def _load_binance_perps_34_panel(start: str, end: str) -> dict[str, pd.DataFrame]:
    """Multi-asset Binance perp panel from the local immutable data store.

    Expects 1h OHLCV CSVs produced by ``research.cli download`` under
    ``agent/research/data/``. Symbols without >= 90% coverage of the common
    timestamp window are dropped, and the final panel keeps only rows where
    every selected symbol has a full OHLCV bar. This gives the zoo a clean
    cross-section for IC benches.
    """
    raw = _BINANCE_PERPS_34_UNIVERSE_FILE.read_text(encoding="utf-8")
    symbols = [
        s.strip()
    ...
    end_exclusive = (datetime.fromisoformat(end) + timedelta(days=1)).replace(
        tzinfo=timezone.utc
    )
    ...
    fetched: dict[str, pd.DataFrame] = {}
    for sym in symbols:
        df = load_bars(sym, _BINANCE_PERPS_34_TIMEFRAME, data_dir=DATA_DIR)
        if df.empty:
            logger.warning("binance_perps_34: no stored bars for %s", sym)
            continue
        df = df[(df["ts"] >= start_ms) & (df["ts"] < end_ms)].copy()
        if df.empty:
            logger.warning(
                "binance_perps_34: %s has no bars in %s..%s", sym, start, end
            )
            continue
    ...
    if not fetched:
        raise RuntimeError(
            "binance_perps_34: no symbols could be loaded from research/data; "
            "run the research download CLI first for the universe list"
        )

    panel = _wide_from_fetched(fetched, include_amount=False)
    # Prune to a common, complete cross-section.
    close = panel["close"]
    keep_cols = close.columns[
        close.notna().mean() >= _BINANCE_PERPS_MIN_COVERAGE
    ].tolist()
    if not keep_cols:
        raise RuntimeError(
            "binance_perps_34: no symbol has enough non-NaN closes after pruning"
        )
    required_fields = ["open", "high", "low", "close", "volume"]
    complete_rows = pd.Series(True, index=close.index)
    for field in required_fields:
        field_panel = panel.get(field)
        if not isinstance(field_panel, pd.DataFrame):
            raise RuntimeError(f"binance_perps_34: required field {field!r} is missing")
        complete_rows &= field_panel[keep_cols].notna().all(axis=1)
    close = close.loc[complete_rows, keep_cols]
    if close.empty:
        raise RuntimeError(
            "binance_perps_34: no common complete OHLCV timestamps across selected symbols"
        )
    if close.shape[1] < 2:
        raise ValueError(
            f"binance_perps_34: only {close.shape[1]} instrument(s) survived "
            "pruning; cross-sectional IC needs >=2"
        )

    panel = {
        k: v.reindex(index=close.index, columns=close.columns)
    ...
    panel["_meta"] = {
        ...
        "timeframe": _BINANCE_PERPS_34_TIMEFRAME,
...
def _select_alpha_ids(
    ...
    if alpha_id:
        ...
        return [alpha_id]
    if zoo:
        zoo_ids = [value.strip() for value in zoo.split(",") if value.strip()]
        if not zoo_ids:
            raise ValueError("zoo selection is empty")
        selected = {
            selected_id
            for zoo_id in zoo_ids
            for selected_id in registry.list(zoo=zoo_id)
        }
        if not selected:
            raise ValueError(f"no alphas found for zoo selection {zoo!r}")
...
class AlphaBenchTool(BaseTool):
    ...
    parameters = {
        ...
        "properties": {
            ...
            "zoo": {
                "type": "string",
                "description": "Bench one or more comma-separated zoos (mutually exclusive with alpha_id).",
            },
            "universe": {
                "type": "string",
                "description": "csi300 | sp500 | btc-usdt | binance_perps_34 (resolved via existing data tools).",
            },
            "period": {
                "type": "string",
