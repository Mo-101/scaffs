...
import { DevRuntimeBanner } from "@/components/layout/DevRuntimeBanner";
...
export function Layout() {
  ...
  const NAV = [
    { to: "/", icon: Home, label: t('layout.home') },
  ...
  return (
    <div className="relative flex h-screen bg-background rtl:flex-row-reverse">
      ...
      {/* Sidebar */}
      <aside className={cn(
        "glass-panel relative z-10 border-e flex flex-col shrink-0 transition-all duration-200 overflow-visible",
        collapsed ? "w-12" : "w-64"
      )}>
        {/* Brand */}
          ...
          {NAV.map(({ to, icon: Icon, label }) => {
            ...
            return (
              <Link
                ...
                to={to}
                className={cn(
                  "glass-nav-link flex items-center text-sm transition-colors",
                  collapsed ? "justify-center p-2" : "gap-3 px-3 py-2",
                  (to === "/" ? pathname === "/" : pathname.startsWith(to))
                    ? "glass-nav-link--active text-primary font-medium"
                  ...
                  <div key={s.session_id} className="group relative flex items-center">
                    {isRenaming ? (
                      <input
                        ...
                        className="glass-input flex-1 min-w-0 ps-3 pe-2 py-1 text-xs"
                      />
                    ) : (
                      <Link
                        to={`/agent?session=${s.session_id}`}
                        className={cn(
                          "glass-nav-link flex-1 min-w-0 ps-3 pe-14 py-1.5 text-xs transition-colors truncate block border-s-2",
                          isActive
                            ? "glass-nav-link--active border-s-primary text-primary font-medium"
                    ...
                    {!isRenaming && isDeleting ? (
                      <div className="absolute right-0.5 flex items-center gap-0.5">
                        <button onClick={() => deleteSession(s.session_id)} className="glass-btn glass-btn--danger glass-btn--sm text-[10px] font-medium">{t('layout.confirm')}</button>
                        <button onClick={() => setDeleteTarget(null)} className="glass-btn glass-btn--ghost glass-btn--sm text-[10px]">{t('layout.cancel')}</button>
                      </div>
                    ) : !isRenaming ? (
                      <div className="absolute right-1 opacity-0 group-hover:opacity-100 flex items-center gap-0.5 transition-opacity">
                        <button
                          ...
                          className="glass-icon-btn h-7 w-7 text-muted-foreground hover:text-foreground"
                        ...
                        <button
                          ...
                          className="glass-icon-btn h-7 w-7 text-muted-foreground hover:text-danger"
        ...
        <div className={cn("border-t", collapsed ? "p-1 flex flex-col items-center gap-1" : "p-3 space-y-2")}>
          {collapsed ? (
            <>
              <button onClick={toggle} className="glass-icon-btn h-8 w-8 text-muted-foreground hover:text-foreground" title={dark ? t('layout.light') : t('layout.dark')}>
                {dark ? <Sun className="h-3.5 w-3.5" /> : <Moon className="h-3.5 w-3.5" />}
              </button>
              <button onClick={() => setCollapsed(false)} className="glass-icon-btn h-8 w-8 text-muted-foreground hover:text-foreground" title={t('layout.expand')}>
            ...
            <>
              <div className="flex items-center justify-between">
                <button
                  onClick={toggle}
                  className="glass-btn glass-btn--ghost glass-btn--sm text-xs text-muted-foreground hover:text-foreground"
                >
                  {dark ? <Sun className="h-3.5 w-3.5" /> : <Moon className="h-3.5 w-3.5" />}
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className="glass-icon-btn h-8 w-8 text-muted-foreground hover:text-foreground"
    ...
    <div>
      ...
      {open && menuStyle && (
        <ul
          ...
          className="glass-menu rounded-xl p-1"
        >
            ...
            return (
              <li key={lang.code}>
                <button
                  ...
                  className={cn(
                    "glass-menu-item w-full flex items-center gap-2 px-2.5 py-1.5 text-xs hover:text-foreground transition-colors",
