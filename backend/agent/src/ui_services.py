...
def load_run_context(run_dir: Path) -> Dict[str, Any]:
    ...
    source = str(context.get("source") or request_data.get("source") or "").strip()
...
def reconstruct_price_series(run_dir: Path) -> List[Dict[str, Any]]:
    ...
    try:
        source = str(context.get("source") or "auto").strip().lower()
        if source == "tushare":
            source = "auto"
        data_map = _fetch_reconstructed_data(codes, fetch_start_date, end_date, source)
...
def _fetch_reconstructed_data(
    codes: List[str],
    start_date: str,
    end_date: str,
    source: str,
) -> Dict[str, Any]:
    """Fetch chart reconstruction rows through registry-backed loaders."""
    from backtest.engines._market_hooks import _detect_market
    from backtest.loaders.registry import VALID_SOURCES, get_loader_cls_with_fallback, resolve_loader
    from backtest.source_schema import CRYPTO_SOURCES

    def normalize(loader_name: str, group: List[str]) -> List[str]:
        if loader_name in CRYPTO_SOURCES:
            return [code.replace("/", "-").upper() for code in group]
        return group

    if source != "auto" and source in VALID_SOURCES:
        Loader = get_loader_cls_with_fallback(source)
        loader = Loader()
        loader_name = getattr(loader, "name", source)
        return loader.fetch(normalize(loader_name, codes), start_date, end_date)

    market_groups: Dict[str, List[str]] = {}
    for code in codes:
        market_groups.setdefault(_detect_market(code), []).append(code)

    merged: Dict[str, Any] = {}
    for market, group in market_groups.items():
        loader = resolve_loader(market)
        loader_name = getattr(loader, "name", "auto")
        merged.update(loader.fetch(normalize(loader_name, group), start_date, end_date))
    return merged
