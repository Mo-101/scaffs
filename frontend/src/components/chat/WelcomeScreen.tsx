    ...
    <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-8 text-center">
      ...
      <div className="flex flex-wrap justify-center gap-2 max-w-lg">
        {CAPABILITY_CHIP_KEYS.map((key) => (
          <span key={key} className="glass-chip text-xs text-muted-foreground">
            {t(key)}
          </span>
        ))}
      </div>
              ...
              <div className="space-y-1.5">
                {cat.examples.map((ex) => (
                  <button
                    ...
                    className={`glass-surface glass-card block w-full text-left px-3 py-2.5 rounded-xl border transition-colors ${cat.color}`}
                  >
