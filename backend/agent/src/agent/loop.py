...
from src.core.run_card_gate import run_card_is_valid
...
STREAM_MAX_RETRIES = int(os.getenv("VT_STREAM_MAX_RETRIES", "3"))
STREAM_RETRY_BACKOFF = float(os.getenv("VT_STREAM_RETRY_BACKOFF", "2.0"))
...
def _canonical_tool_call_key(name: str, arguments: dict[str, Any]) -> tuple[str, str]:
    """Stable (tool_name, args) key so different arguments are never treated as a duplicate.

    Keying dedup on tool name alone meant a second call to a read-many tool
    (e.g. get_market_data for a different symbol) would be served the FIRST
    call's cached result -- silently wrong data, not a cache hit.
    """
    try:
...
class AgentLoop:
    ...
    def __init__(
        ...
        self._tool_call_repeat_count: dict[tuple[str, str], int] = {}
        self._repeated_tool_loop_reason: str | None = None
    ...
    def run(self, user_message: str, history: Optional[List[Dict[str, Any]]] = None, session_id: str = "") -> Dict[str, Any]:
        ...
        self._tool_call_repeat_count = {}
        self._repeated_tool_loop_reason = None
        ...
        try:
            while iteration < self.max_iterations:
                ...
                def _on_llm_heartbeat(payload: Dict[str, Any]) -> None:
                    self._emit("llm_heartbeat", {**payload, "iter": current_iter})
                ...
                while True:
                    try:
                        ...
                        # mid-stream failures (connection reset, relay
                        # hiccup, provider still starting up) — mirrors the
                        # swarm worker policy. Deterministic 4xx errors fail
                        # immediately. Deltas from the failed attempt are
                        # dropped so the trace does not contain duplicated
                        # thinking text.
                        if not exc.retryable or attempt >= STREAM_MAX_RETRIES:
                            raise
                        delay = STREAM_RETRY_DELAY_S * (STREAM_RETRY_BACKOFF ** attempt)
                        attempt += 1
                        logger.warning(
                ...
                if self._repeated_tool_loop_reason:
                    break
        ...
        if self._cancel_event.is_set():
            ...
            state_store.mark_failure(run_dir, final_reason)
            final_status = "failed"
        elif self._repeated_tool_loop_reason:
            # A tool succeeded, then got called again with identical arguments
            # more than twice. That is not the model incorporating a receipt --
            # it is a loop that never advances. Fail loudly rather than let
            # eventual narration (final_content below) pass as success.
            final_reason = f"agent_loop_repeated_successful_tool_result: {self._repeated_tool_loop_reason}"
            state_store.mark_failure(run_dir, final_reason)
            final_status = "failed"
        elif "backtest" in self._called_ok and not run_card_is_valid(run_dir):
            # A governed backtest was invoked this run: success is only real
            # when run_card.json exists with its truth fields. The model's
            # narration is not itself evidence the backtest completed.
            final_reason = "governed_backtest_missing_run_card"
            state_store.mark_failure(run_dir, final_reason)
            final_status = "failed"
    ...
    def _process_tool_calls(
        ...
        for tc in tool_calls:
            ...
            call_key = _canonical_tool_call_key(tc.name, tc.arguments)
            if call_key in self._tool_call_cache and not is_repeatable:
                repeat_count = self._tool_call_repeat_count.get(call_key, 0) + 1
                self._tool_call_repeat_count[call_key] = repeat_count

                # Hard loop breaker: the same tool called with the same args after
                # it already succeeded, more than twice, is not the executor
                # incorporating a receipt -- it's cognition that never advances.
                if repeat_count > 2:
                    self._repeated_tool_loop_reason = (
                        f"{tc.name} called with identical arguments "
                        f"{repeat_count + 1} times after it already succeeded"
                    )
                    trace.write({
                        "type": "agent_loop_repeated_successful_tool_result",
                        "iter": iteration, "tool": tc.name, "repeat_count": repeat_count + 1,
                    })
                    return compact_requested, focus_topic

                logger.warning(f"Duplicate call to {tc.name} (same args, already succeeded) — returning cached result")
                cached_result = self._tool_call_cache.get(call_key, "")
                messages.append(context.format_tool_result(tc.id, tc.name, cached_result))
                trace.write({"type": "tool_skipped", "iter": iteration, "tool": tc.name, "cached": True})
    ...
    def _finalize_tool_result(
        ...
        if success:
            ...
            self._tool_call_cache[_canonical_tool_call_key(tc.name, tc.arguments)] = truncated
