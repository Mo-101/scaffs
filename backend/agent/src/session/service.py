...
class SessionService:
    ...
    async def _run_with_agent(
        ...
        *,
        ...
        # Cap the tool set for small local models that cannot digest 60+ tool schemas.
        enabled_tools_env = os.environ.get("VIBE_TRADING_ENABLED_TOOLS")
        if enabled_tools_env is not None:
            from src.tools import _filter_registry

            wanted = [n.strip() for n in enabled_tools_env.split(",") if n.strip()]
            registry = _filter_registry(registry, wanted, include_shell_tools=include_shell_tools)
