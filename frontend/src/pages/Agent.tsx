...
import { Send, Loader2, ArrowDown, Square, Download, Plus, Paperclip, X, Users, Target, ChevronDown, Pencil, Check, Play, OctagonX, Activity, Ban, CheckCircle2, Landmark, ShieldCheck } from "lucide-react";
...
import { GovernedBacktestModal, type GovernedBacktestParams } from "@/components/chat/GovernedBacktestModal";
import { RotatingWorkingStatus } from "@/components/chat/RotatingWorkingStatus";
...
export function Agent() {
  ...
  const [showGovernedBacktestModal, setShowGovernedBacktestModal] = useState(false);
  const [governedBacktestSubmitting, setGovernedBacktestSubmitting] = useState(false);
  ...
  const setupSSE = useCallback((sid: string) => {
    ...
    connect(api.sseUrl(sid, { replay: "active" }), {
      ...
      heartbeat: () => {},
      // Emitted while the LLM stream is still awaiting its first token/chunk
      // (no tool involved yet) -- keeps the idle timer alive so a legitimately
      // slow model isn't mistaken for a stuck connection.
      llm_heartbeat: () => { touch(); },
  ...
  const submitGovernedBacktest = async (params: GovernedBacktestParams) => {
    setGovernedBacktestSubmitting(true);
    act().addMessage({
      id: "", type: "user",
      content: `[Governed backtest — deterministic, no chat model] ${params.symbols.join(", ")} · ${params.start_date} → ${params.end_date} · ${params.interval} · source=binance`,
      timestamp: Date.now(),
    });
    forceScrollToBottom();
    try {
      const result = await api.runGovernedBacktest(params);
      if (result.status !== "success") {
        act().addMessage({
          id: "", type: "error",
          content: `Governed backtest failed: ${result.reason || "unknown reason"}. No run_card.json was produced, so no success is reported.`,
          timestamp: Date.now(),
        });
        return;
      }
      setShowGovernedBacktestModal(false);
      let runMetrics: Record<string, number> | undefined;
      let runCurve: Array<{ time: string; equity: number }> | undefined;
      try {
      ...
      const message = isAuthRequiredError(error) ? AUTH_REQUIRED_MESSAGE : (error instanceof Error ? error.message : "Governed backtest request failed");
      toast.error(message);
      act().addMessage({ id: "", type: "error", content: message, timestamp: Date.now() });
    } finally {
      setGovernedBacktestSubmitting(false);
      scrollToBottom();
    }
  };
  ...
  return (
    <div className="relative flex flex-col flex-1 min-w-0 overflow-hidden h-full">
      <div ref={listRef} className="relative z-10 flex-1 overflow-auto p-6 scroll-smooth">
      ...
      <form onSubmit={handleSubmit} className="glass-surface glass-surface--accent-edge relative z-10 p-4">
          ...
          <div className="flex gap-2 items-end">
            ...
            <div className="relative" ref={uploadMenuRef}>
              <button
                ...
                className="glass-icon-btn h-9 w-9 text-muted-foreground hover:text-foreground disabled:opacity-40 shrink-0"
                title={t("agent.moreOptions")}
              >
                <Plus className="h-4 w-4" />
              </button>
              {showUploadMenu && (
                <div className="glass-menu absolute bottom-full left-0 mb-2 w-52 rounded-xl p-1 z-50">
                  <button
                    type="button"
                    onClick={() => { fileInputRef.current?.click(); setShowUploadMenu(false); }}
                    className="glass-menu-item w-full px-3 py-2 text-left text-sm transition-colors flex items-center gap-2"
                  >
                  ...
                  <button
                    ...
                    onClick={() => {
                      setShowUploadMenu(false);
                      setShowGovernedBacktestModal(true);
                    }}
                    className="glass-menu-item w-full px-3 py-2 text-left text-sm transition-colors flex items-center gap-2"
                  >
            ...
            <textarea
              ...
              className="glass-textarea flex-1 text-sm max-h-32 overflow-y-auto"
              disabled={status === "streaming"}
            />
            {messages.length > 0 && (
              <button
                ...
                className="glass-btn glass-btn--ghost px-3 py-2.5 text-muted-foreground hover:text-foreground"
            ...
            {status === "streaming" ? (
              <button
                ...
                className="glass-btn glass-btn--danger px-4 py-2.5 text-sm font-medium"
              ...
              <button
                ...
                className="glass-btn glass-btn--primary px-4 py-2.5 text-sm font-medium disabled:opacity-40"
              >
      ...
      <GovernedBacktestModal
        open={showGovernedBacktestModal}
        submitting={governedBacktestSubmitting}
        onClose={() => setShowGovernedBacktestModal(false)}
        onSubmit={submitGovernedBacktest}
      />
    </div>
  );
}
