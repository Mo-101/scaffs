...
# ---------------------------------------------------------------------------
# Paper trading ledger-audit tools
#
# Read-only wrappers around paper_session.py's own diagnostics -- these call
# the exact compute_session_status / compute_session_diagnostics functions
# the frontend and CLI already use, so an MCP client sees the identical
# numbers rather than a separately-computed approximation. Every session_id
# must resolve inside paper_session.SESSIONS_DIR; no arbitrary filesystem
# paths are accepted.
# ---------------------------------------------------------------------------


def _resolve_paper_session_dir(session_id: str):
    """Resolve a bare session name to its directory under paper_sessions/.

    Rejects any path that escapes SESSIONS_DIR (rules out '..' traversal or
    an absolute path pointed elsewhere) -- callers only ever get to name a
    session that already exists on disk in this project.
    """
    import paper_session

    sessions_root = paper_session.SESSIONS_DIR.resolve()
    candidate = (sessions_root / session_id).resolve()
    if candidate != sessions_root and sessions_root not in candidate.parents:
        raise ValueError(f"session_id must resolve inside {sessions_root}")
    if not (candidate / "session.json").exists():
        raise FileNotFoundError(f"no session.json found for '{session_id}'")
    return candidate


@mcp.tool
def list_paper_trading_sessions() -> str:
    """List every paper-trading session with its real runtime/accounting status.

    Reads session directories under agent/paper_sessions/ (excludes
    _archived/). Each row is paper_session.compute_session_status()'s own
    output -- runtime_status (running/stopped, from the session's live
    heartbeat), accounting_status (OK / ACCOUNTING_ERROR, set by the
    session's own reconciliation guard), session_role (control/candidate/
    historical) and regimen (e.g. "5m"). No synthetic or estimated data: a
    session with no marks yet reports runtime_status="unknown" rather than
    a fabricated value.
    """
    import paper_session

    sessions_dir = paper_session.SESSIONS_DIR
    if not sessions_dir.exists():
        return _json_ok(sessions=[])
    rows = []
    for d in sorted(
        p for p in sessions_dir.iterdir()
        if p.is_dir() and p.name != "_archived" and (p / "session.json").exists()
    ):
        try:
            status = paper_session.compute_session_status(d)
            session = paper_session._load_session(d)
        except Exception as exc:  # noqa: BLE001 - one bad session shouldn't abort the list
            rows.append({"session_id": d.name, "error": str(exc)})
            continue
        rows.append({
            "session_id": d.name,
    ...
    return _json_ok(sessions=rows)


@mcp.tool
def audit_paper_trading_session(session_id: str) -> str:
    """Run the real accounting/reconciliation audit on one paper-trading session.

    ``session_id`` is the session's directory name under
    agent/paper_sessions/ (e.g. "shadow_ab_v2_candidate10_5m"), not a
    filesystem path. Returns paper_session.compute_session_diagnostics()'s
    full metrics (realized/unrealized P&L, fees, drawdown,
    reconciliation_error, reconciled flag) plus the session's own
    accounting_status / accounting_error_kind if its invariant guard has
    ever frozen it. This calls the same reconciliation logic
    paper_session.py itself uses to halt a session on a broken ledger
    (position_ledger_differences + assess_accounting) rather than
    reimplementing or approximating it.

    Args:
        session_id: Directory name under agent/paper_sessions/.
    """
    import paper_session

    try:
        session_dir = _resolve_paper_session_dir(session_id)
    except (ValueError, FileNotFoundError) as exc:
        return _json_error(str(exc), error_type="not_found")

    try:
        diagnostics = paper_session.compute_session_diagnostics(session_dir)
        status = paper_session.compute_session_status(session_dir)
    except Exception as exc:  # noqa: BLE001
        return _json_error(f"failed computing diagnostics for '{session_id}': {exc}", error_type="audit_error")
    ...
    return _json_ok(
        ...
        accounting_error_kind=session.get("accounting_error_kind"),
        accounting_position_differences=session.get("accounting_position_differences"),
        metrics=diagnostics["metrics"],
        by_symbol=diagnostics["by_symbol"],
        trade_count=diagnostics["trade_count"],
        closed_trade_count=diagnostics["closed_trade_count"],
    )
