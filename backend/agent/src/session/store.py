...
_SERVER_EPOCH_FILE_NAME = ".server_epoch"
...
class SessionStore:
    ...
    def __init__(self, base_dir: Path) -> None:
        ...
        # Regular attempts have no heartbeat trail (unlike swarm runs' events.jsonl),
        # so silence-based staleness detection isn't available here. Instead, stamp
        # this process's boot time once at construction: any attempt still marked
        # "running" but created before THIS server started must belong to a process
        # that's already gone - no restart can un-orphan it - while a genuinely
        # long-running attempt under the same live process is never a false positive.
        self._server_epoch = datetime.now()
        self._write_server_epoch_marker()

    def _write_server_epoch_marker(self) -> None:
        """Persist this process's boot time for operator debugging.

        Not read back by :meth:`is_attempt_stale` (the in-memory
        ``_server_epoch`` is authoritative for this process's own lifetime) -
        kept on disk so an operator can ``cat`` it to confirm when the
        currently-running server actually started.
        """
        try:
            (self.base_dir / _SERVER_EPOCH_FILE_NAME).write_text(
                self._server_epoch.isoformat(), encoding="utf-8"
            )
        except OSError:
            logger.warning("could not persist server epoch marker", exc_info=True)
    ...
    def is_attempt_stale(self, attempt: Attempt) -> bool:
        """Return True when a ``running`` attempt predates this server process.

        A killed/restarted backend leaves in-flight attempts permanently
        marked ``running`` on disk with no process left to ever complete
        them. This never mutates the stored status - callers surface it as
        a read-only flag alongside the real status, the same contract the
        swarm engine's ``is_run_stale`` uses for its own runs.

        Args:
            attempt: Attempt to check.

        Returns:
            True if ``attempt`` is still ``running`` but was created before
            this server process booted. False for any other status, and
            False (never a false positive) for a long-running attempt
            created under this same live process.
        """
        ...
        try:
            ...
            return False
        return created < self._server_epoch
