...
def _default_file_roots() -> list[Path]:
    ...
    return [
        ...
        # Mirrors allowed_write_roots(): autopilot run dirs must stay
        # readable by the same tool call pattern that can write to them,
        # even when the caller passes a stale/mismatched run_dir.
