...
def detect_source(code: str) -> str:
    ...
    The catch-all for symbols matching none of the known patterns is
    ``akshare``, not ``tushare``: OHLCV auto-routing should never default to
    a token-gated provider when a free one covers the same ground. Tushare
    stays fully available — via explicit ``source="tushare"``, a configured
    ``TUSHARE_TOKEN``, or Tushare-only ``fundamental_fields`` requests — it
    just isn't the assumed guess for an unrecognized format.
    """
...
def fetch_market_data(
    *,
    ...
    The return payload carries a structured verdict alongside the data —
    ``_sources_used`` (which provider actually resolved each symbol) and
    ``_unresolved_reasons`` (why an unresolved symbol failed) — so a caller
    (including an LLM reading the tool result) never has to infer success
    or failure from a bare symbol list or an incidental warning some
    provider SDK printed to stdout. Both keys are additive: the happy-path
    payload for existing callers only gains ``_sources_used``, and
    ``_unresolved``/``_unresolved_reasons`` are omitted entirely when every
    code resolves.
    """
    ...
    for src, src_codes in groups.items():
        ...
        try:
            ...
            loader = loader_cls()
            resolved_name = getattr(loader, "name", src)
            data_map = loader.fetch(src_codes, start_date, end_date, interval=interval)
        except Exception as exc:  # noqa: BLE001 - contained, surfaced via _unresolved_reasons
            logger.exception(
            ...
            data_map = {}
            group_errors[src] = f"{resolved_name} loader failed: {exc}"
        for symbol, df in data_map.items():
            ...
            sources_used[symbol] = resolved_name

    unresolved = [code for code in codes if code not in results]
    if unresolved:
        results["_unresolved"] = unresolved
        code_to_group_src = {
            code: src for src, src_codes in groups.items() for code in src_codes
        }
        results["_unresolved_reasons"] = {
            code: group_errors.get(code_to_group_src.get(code, ""), "loader returned no data for this symbol")
