    ...
    <div className="p-8 max-w-4xl space-y-6">
      ...
      <div className="flex gap-4 items-end">
        <div className="flex-1">
          ...
          <select value={leftId} onChange={(e) => setLeftId(e.target.value)} className="glass-select text-sm" title={leftRun?.prompt || leftId}>
        ...
        <div className="flex-1">
          ...
          <select value={rightId} onChange={(e) => setRightId(e.target.value)} className="glass-select text-sm" title={rightRun?.prompt || rightId}>
            <option value="">{i18n.t("compare.select")}</option>
            {runs.map((r) => <option key={r.run_id} value={r.run_id}>{runLabel(r)} ({r.status})</option>)}
          </select>
        </div>
