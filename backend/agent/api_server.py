...
# Allow direct `python api_server.py` invocations to be discovered as `api_server`.
_sys.modules.setdefault("api_server", _sys.modules[__name__])

RUNS_DIR = Path(__file__).resolve().parent / "runs"
AUTOPILOT_RUNS_DIR = Path.home() / ".vibe-trading" / "runs"
...
class AutopilotEvidenceRunInfo(BaseModel):
    """Compact row for a provenance-valid autopilot run."""
    run_dir: str
    ...
    signal_engine_sha256: Optional[str] = None
...
class SessionResponse(BaseModel):
    ...
    last_attempt_is_stale: bool = False
...
# ----------------------------------------------------------------------------
# Reverse proxy for paper-dashboard API (paper_dashboard_api.py on :8787)
# ----------------------------------------------------------------------------
# The frontend PaperTradingDashboard page calls same-origin /api/portfolio,
# /api/status, /api/sessions, /api/positions/close, /api/session/close-all.
# In dev mode Vite proxies these to :8787; in prod (serving dist from this
# server) we need to forward them here.
import httpx

_PAPER_DASH_TARGET = os.getenv("PAPER_DASH_URL", "http://127.0.0.1:8787")
_PAPER_DASH_ROUTES = frozenset({
    "/api/portfolio",
    "/api/sessions",
    "/api/status",
    "/api/positions/close",
...
_API_KEY_FILE_ENV = "API_AUTH_KEY_FILE"
_SHELL_TOOLS_ENV = "VIBE_TRADING_ENABLE_SHELL_TOOLS"
_DOCKER_LOOPBACK_ENV = "VIBE_TRADING_TRUST_DOCKER_LOOPBACK"


def _configured_api_key() -> str:
    """Return the current API auth key without exposing file-backed secrets.

    Production deployments mount the dashboard credential as a Docker secret
    file. The environment value remains supported for local development.
    """
    configured = os.getenv("API_AUTH_KEY") or _API_KEY
    if configured:
        return configured
    key_file = os.getenv(_API_KEY_FILE_ENV, "").strip()
    if not key_file:
        return ""
    try:
...
@app.get(
    "/autopilot-runs",
    response_model=List[AutopilotEvidenceRunInfo],
    dependencies=[Depends(require_auth)],
)
async def list_autopilot_evidence_runs(limit: int = 50):
    """List autopilot runs with provenance-valid evidence.

    Reads ``run_card.json`` from each ``~/.vibe-trading/runs/autopilot_*``
    directory and returns only runs where ``provenance_valid`` is ``true``
    (real, non-scaffold artifacts — not a claim that the hypothesis is
    supported; see ``hypothesis_supported`` on each row for that). Distinct
    from ``/runs``, which serves session-local ``agent/runs`` entries.
    """
    limit = min(max(1, limit), 200)
    runs_dir = AUTOPILOT_RUNS_DIR

    if not runs_dir.exists():
        return []

    run_dirs = sorted(
        (d for d in runs_dir.iterdir() if d.is_dir() and d.name.startswith("autopilot_")),
        key=lambda x: x.name,
        reverse=True,
    )

    results: List[AutopilotEvidenceRunInfo] = []
    for d in run_dirs:
        if (d / "QUARANTINED").exists():
            continue
        card = _load_json_file(d / "run_card.json")
        if not card or card.get("provenance_valid") is not True:
            continue

        metrics = card.get("metrics") or {}
        results.append(AutopilotEvidenceRunInfo(
            run_dir=str(d),
            strategy_implementation_status=str(card.get("strategy_implementation_status") or "missing"),
            provenance_valid=True,
            has_trades=bool(card.get("has_trades")),
            statistically_evaluable=bool(card.get("statistically_evaluable")),
            hypothesis_supported=card.get("hypothesis_supported")
            if isinstance(card.get("hypothesis_supported"), bool)
            else None,
            run_purpose=str(card.get("run_purpose") or "missing"),
            trade_count=metrics.get("trade_count"),
            total_return=metrics.get("total_return"),
            sharpe=metrics.get("sharpe"),
            generated_at=card.get("generated_at"),
            signal_engine_sha256=card.get("signal_engine_sha256"),
        ))
...
def _last_attempt_fields(svc: Any, session_id: str, last_attempt_id: Optional[str]) -> tuple[Optional[str], bool]:
    """Resolve a session's last-attempt status and staleness for the API response.

    Returns:
        ``(last_attempt_status, last_attempt_is_stale)`` - both default to a
        harmless empty state (``None``, ``False``) when there is no attempt
        yet or its record can't be read, so a missing/corrupt attempt file
        never breaks the surrounding session response.
    """
    if not last_attempt_id:
        return None, False
    attempt = svc.store.get_attempt(session_id, last_attempt_id)
    if attempt is None:
        return None, False
    return attempt.status.value, svc.store.is_attempt_stale(attempt)
...
async def list_sessions(limit: int = Query(50, ge=1, le=200)):
    ...
    result = []
    for s in sessions:
        last_attempt_status, last_attempt_is_stale = _last_attempt_fields(svc, s.session_id, s.last_attempt_id)
        result.append(SessionResponse(
            ...
            last_attempt_is_stale=last_attempt_is_stale,
        ))
    return result
...
async def get_session(session_id: str):
    ...
    if not session:
        raise HTTPException(status_code=404, detail=f"Session {session_id} not found")
    last_attempt_status, last_attempt_is_stale = _last_attempt_fields(
        svc, session.session_id, session.last_attempt_id
    )
    return SessionResponse(
        session_id=session.session_id,
        title=session.title,
...
def _build_live_runner(broker: str) -> Any:
    ...
    svc = _get_session_service()
    if svc is None:
        raise LiveRunnerUnavailable("session runtime is not enabled (ENABLE_SESSION_RUNTIME=false)")
...
# ============================================================================
# Governed Backtest routes (deterministic, no ReAct) — src/api/governed_backtest_routes.py
# ============================================================================

from src.api.governed_backtest_routes import register_governed_backtest_routes  # noqa: E402
register_governed_backtest_routes(app)

# ============================================================================
# Paper Session routes (read-only; simulated live paper trading) — src/api/paper_session_routes.py
# ============================================================================

from src.api.paper_session_routes import register_paper_session_routes  # noqa: E402
register_paper_session_routes(app)

# ============================================================================
# IdimIkang data-source routes (observer-only ingestion)
# ============================================================================

from src.api.idimikang_routes import register_idimikang_routes  # noqa: E402
register_idimikang_routes(app)
...
def serve_main(argv: list[str] | None = None) -> int:
    ...
    if args.dev and frontend_root.exists():
        ...
        print(
            "[warn] Serving a prebuilt frontend/dist bundle. If you're developing, "
            "this UI can be STALE (frozen at the last `npm run build`). Restart with "
            "--dev to run the live Vite dev server on :5899 instead."
        )
    else:
