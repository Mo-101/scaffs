...
function fallbackAuthorizeInstruction(): string {
  return "Run `scaffs connector list`, choose the broker profile, then run `scaffs connector authorize <profile>` from the desktop session that will hold the broker connection.";
}
...
function BrokerRow({
  ...
  return (
    <div className="glass-panel grid gap-2 rounded-xl p-2.5">
  ...
  return (
    <div className="grid gap-2">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="glass-chip glass-chip--interactive max-w-full justify-self-start text-left text-xs font-medium text-primary"
